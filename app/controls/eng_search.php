...
$sql = "SELECT * FROM engenheiros where nome like '%$busca%' AND ativo = '1'";
$resultado = mysqli_query($conn, $sql);
$total_linhas = mysqli_num_rows($resultado);
$sql = "SELECT * FROM engenheiros where nome like '%$busca%' AND ativo = '1' order by nome ASC LIMIT $limit OFFSET $offset";
    ...
    <table>
        ...
        <?php while ($row = mysqli_fetch_assoc($res)) : ?>
            <tr>
                ...
                <td>
                    <a href="../views/edit_user?id=<?php echo $row['id_eng'] ?>"><span title="Editar"><button>Editar</span></button></a>
                    <a onclick="return confirm('Deseja realmente excluir esse engenheiro?')" href="../controls/delete_eng?id=<?php echo $row['id_eng'] ?>"><button>Desativar</button></a>
                </td>
